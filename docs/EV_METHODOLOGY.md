...
### 実データでの中間結果（2016年・データ量の効果）

実 netkeiba から収集して walk-forward 検証した実測。**データを増やすほど独立モデルが市場に近づく**
ことが確認できた（＝手法は正しく機能し、現状はデータ量・特徴量の不足がボトルネック）。

| 収集期間 | レース数 | 馬の平均出走数 | 平均ΔR²（市場超過エッジ） | α（自モデル） |
|---|---|---|---|---|
| 2ヶ月（5–6月） | 540 | 1.54 | −0.100 | 0.15–0.31 |
| 6ヶ月（1–6月） | 1,704 | 3.18 | **−0.0017（ほぼ0）** | 0.03–0.16 |

- ΔR² が −0.10 → ほぼ 0 へ改善＝**過去走が蓄積するほど特徴量が効き、市場に追いつく**。予測どおりの挙動。
- ただし **まだ市場を「超えて」いない**（ΔR²>0 に届かず、α も小さい）。少量データで偽の EV>1 を出さない
  正しい挙動でもある。
- **EV>1.0 への残タスクは明確**: ①10年級のデータ（Benter は 5 年以上）で各馬の履歴を厚くする、
  ②特徴量の拡充（血統・詳細ラップ/クラス・馬場や距離との交互作用・近走の時間減衰重み）。
  現状の17特徴量（結果ページ由来）では独立情報が足りない。
